...
num_samples = 2 ** 12
train_data = pd.DataFrame(torch.load('/home/samiri/SynDG/normalizing-flows/examples/maf_train.pt'))
test_data = pd.DataFrame(torch.load('/home/samiri/SynDG/normalizing-flows/examples/maf_test.pt') )
train_dataloader = DataLoader(train_data.values, batch_size=num_samples, shuffle=True)
test_dataloader = DataLoader(test_data.values, batch_size=num_samples, shuffle=True)
# %%
...
dim = 8
for _b in ['ggd mixture','ggd','student t','gaussian']:
    for _betab in [2.]:
        for _vsh in [6.]:
            ...
            try:
                ...
                _l = np.median(train_dataloader.dataset,axis=0)
                _s = np.std(train_dataloader.dataset,axis=0)

            except:
                _l = np.median(train_dataloader.dataset,axis=0).cpu().detach().numpy()
                _s = train_dataloader.dataset.std(axis=0).cpu().detach().numpy()
            ...
            if _b == 'student t':
                base = nf.distributions.base_extended.MultivariateStudentTDist(degree_of_freedom=2.,dim=dim,trainable=True,device='cuda').cuda()
            elif _b == 'ggd mixture':
                trnbl = True
                base = nf.distributions.base_extended.GeneralizedGaussianMixture(n_modes=100, rand_p=True, noise_scale=0.1, dim=dim,loc=_l,scale=_s,p=_betab,device=device,trainable_loc=trnbl, trainable_scale=trnbl,trainable_p=trnbl,trainable_weights=trnbl)    
            elif _b == 'ggd':
                trnbl = True
                base = nf.distributions.base_extended.GeneralizedGaussianMixture(n_modes=1, rand_p=True, noise_scale=0.5, dim=dim,loc=0,scale=1.,p=_betab,device=device,trainable_loc=trnbl, trainable_scale=trnbl,trainable_p=trnbl,trainable_weights=trnbl)    
            elif _b == 'gaussian':
                base = nf.distributions.base.DiagGaussian(dim,trainable=trnbl).cuda()
            ...
            optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=5e-8)
            ...
            bestloss = 1e10
            model.train()
            pbar = tqdm(range(max_iter), desc=f'best error:{bestloss:.6f}')
            for it in pbar:
                # if it == 1000:
                #     optimizer.set_lr(1e-6)
                optimizer.zero_grad()
                pbar.set_description(f'best error:{bestloss:.6f}')
                
                # Get training samples
                x = batch = next(iter(train_dataloader)).type(torch.float32).to(device)
                ...
                try:
                    ...
                    if ~(torch.isnan(loss) | torch.isinf(loss)):
                        ...
                        with torch.no_grad():
                            fds = model.sample(10000)[0].cpu().detach().numpy()
                            mask = ~np.any(np.isnan(fds) | np.isinf(fds), axis=1)
                            if loss.item()<bestloss and mask.sum()>9900:
            ...
            plt.plot(loss_hist, label='loss')
            plt.show()
            torch.save(models,'models_results_mtaf.pt')
...
models = torch.load('../../models_results_mtaf.pt')
...
for m in models:
    ...
    model = m[3]
    target = m[5]
    tds = test_data.values
    fds = model.sample(len(test_data))[0].cpu().detach().numpy()
    mask = ~np.any(np.isnan(fds) | np.isinf(fds), axis=1)
    fds = fds[mask]
    areas = []
    for i in range(8):
