...
class Funnel(Target):
    ...
    def log_prob(self, z):
        ...
        v = z[:,0]
        x = z[:,1]
        v_like = nf.distributions.base_extended.GeneralizedGaussianDistribution(torch.tensor([0.0],device='cuda'), torch.tensor([1.0],device='cuda'),torch.tensor([1000-self.beta+2],device='cuda')).log_prob(v)
            ...
            for _r in [True,False]:
                for _b in ['Gaussian','Generalized Gaussian', 'Mixtures of Generalized Gaussians']:
                    ...
                    if _b == 'Mixtures of Generalized Gaussians': 
                        base = nf.distributions.base_extended.GeneralizedGaussianMixture(n_modes=20, rand_p=True, noise_scale=0.2, dim=2,loc=_l,scale=1.,p=2.,device=device,trainable_loc=trnbl, trainable_scale=trnbl,trainable_p=trnbl,trainable_weights=trnbl).cuda()
                    elif _b == "Student's t":
