...
for nl in [_nl]:
    ...
    # for w in list(reversed([64,128,192,256,378,512,768,1024,2048,4096,8192])):    
            try:
                ...
                torch.save(model,f'./power_{nl}_{w}_{ml}_{lr}_{fltyp}_{rbst}_{vlay}.pt')
                del model
                ...
                ds_gn.to_csv(f'./power_{nl}_{w}_{ml}_{lr}_{fltyp}_{rbst}_{vlay}.csv')
                ...
                plt.tight_layout()
                plt.show()
                plt.savefig(f'./power_{nl}_{w}_{ml}_{lr}_{fltyp}_{rbst}_{vlay}.png')
                del ds_gn
                torch.cuda.empty_cache()
                gc.collect()
                torch.cuda.empty_cache()
