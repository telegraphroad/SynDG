...
categorical = []
my_dataset = CSVDataset('./power.csv',categorical)
...
for nl in [_nl]:
    ...
    # for w in list(reversed([64,128,192,256,378,512,768,1024,2048,4096,8192])):    
            try:
                ...
                ds_gn.to_csv(f'./power_{nl}_{w}_{ml}_{lr}_{fltyp}_{rbst}_{vlay}_{nsamp}_{nmodes}_{rndadd}_{useloc}_{usestd}_{initp}.csv')
                my_dataset.data.to_csv(f'./power_gen.csv')
                ...
                plt.tight_layout()
                plt.show()
                plt.savefig(f'./power_{nl}_{w}_{ml}_{lr}_{fltyp}_{rbst}_{vlay}_{nsamp}_{nmodes}_{rndadd}_{useloc}_{usestd}_{initp}.png')
                del ds_gn
                torch.cuda.empty_cache()
                gc.collect()
                torch.cuda.empty_cache()
